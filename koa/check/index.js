...
const specifyTypeIsPosChecker = landuse=>{
    const positive = ['제1종전용주거지역', '제2종전용주거지역', '제1종일반주거지역', '제2종일반주거지역', '제3종일반주거지역', '보전녹지지역', '생산녹지지역', '자연녹지지역']
    const negative = ['준주거지역', '중심상업지역', '일반상업지역', '근린상업지역', '유통상업지역', '준공업지역']
...
const mapFunctions = {
    국토계획법: specifyTypeIsPos=>specifyTypeIsPos? 
    f=>{
        let gukgye
        if (f.consts.length) {
    ...
    조례: specifyTypeIsPos=>specifyTypeIsPos? 
    f=>{
        let jorye
