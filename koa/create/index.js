...
export const getFacilityList = async (ctx) => {
    ...
    const res = await session.run(
        'match (f:facility) return f.ssCode, f.ssName order by f.ssCode'
    )
...
const createTest = async (ctx) => {
    ...
    try {
        let res = await session.run(
            `match (original:${nodeLabel}) where id(original)=${nodeId} create (bu:${nodeLabel}:backup) set bu=original, bu.timestamp=timestamp()`
        )
        ...
        res = await session.run(
            `match (original:${nodeLabel}) where id(original)=${nodeId} set original+=$props, original.updated_dt = date()`,
            updatingParam
        )
